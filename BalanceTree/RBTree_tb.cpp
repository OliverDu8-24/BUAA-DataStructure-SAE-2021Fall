...
void SimpleFunctionTest() {
    FILE *res_simpletest = fopen("./res/rbtree_simple.txt", "w");
    vector<int> arr;
    int i = 10;
    for(int i = 0; i < 11; i++) {
        ...
        for(int k = 1; k <= 5; k++) {
            ...
            timer_s = clock();
            for(auto i : arr) rbtree_insert(root, i);
            timer_e = clock();
            ...
            arr = generate_random_array(data_mass[i]);
            timer_s = clock();
            for(auto i : arr) rbtree_search(root, i);
            timer_e = clock();
            fprintf(res_simpletest, "%f ", (double)(timer_e - timer_s) / CLOCKS_PER_SEC);
            arr.clear();
            ...
            arr = generate_random_array(data_mass[i]);
            timer_s = clock();
            for(auto i : arr) rbtree_delete(root, i);
            timer_e = clock();
...
// Change the ratio of Delete/Insert ratio, Search/Insert ratio, Measure the time
void CombinationFunctionTest() {
    const int INS_NUM = 1000000;
    FILE *res_combtest = fopen("./res/rbtree_comb.txt", "w");
